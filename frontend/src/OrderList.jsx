...
const OrderList = ({ userToken, updateFlag, currentUserName }) => {
    ...
    const handleReviewDelete = async (reviewId, pId, userName) => {
        if (!window.confirm("삭제하시겠습니까?" + userName)) return;
        try {
    ...
    return (
        <div style={containerStyle}>
            ...
            {loading ? <p>로딩 중...</p> : (
                <div style={gridStyle}>
                    {orders.map(order => (
                        <div key={order.id} onClick={() => fetchOrderDetails(order.id)} style={cardStyle}>
                                        ...
                                        {selectedProductId === pId && (
                                            <div style={reviewSectionStyle}>
                                                ...
                                                <div style={reviewListStyle}>
                                                    {reviewsMap[pId]?.map(rev => (
                                                        <div key={rev.id || index} style={reviewItemStyle}>
                                                            <span><strong>{rev.username}</strong>: {rev.content}</span>
                                                            {(rev.username === currentUserName || true) && (
